import type { WorkerConfig, WorkerInterval } from './types';
...
const VALID_INTERVALS: ReadonlyArray<WorkerInterval> = [
...
function parseInterval(raw: string | undefined, fallback: WorkerInterval): WorkerInterval {
  if (!raw) return fallback;
  const cleaned = raw.trim().toLowerCase() as WorkerInterval;
