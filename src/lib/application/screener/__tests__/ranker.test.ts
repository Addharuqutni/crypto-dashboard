...
describe('rankScreenerResults', () => {
  ...
  it('WAIT with no_trade does not create permission conflict noise', () => {
    const results = [
      makeResult({ symbol: 'BTCUSDT', action: 'WAIT', tradePermission: 'no_trade' }),
    ];
    const ranked = rankScreenerResults(results, defaultSettings);
    expect(rankedAt(ranked, 0).alertBlockReasons).toContain('Action is WAIT');
    expect(
      rankedAt(ranked, 0).alertBlockReasons.some((r) => r.includes('permission'))
    ).toBe(false);
  });
