...
describe('SCREENER_AUDITOR_SYSTEM_PROMPT guardrails', () => {
  it('contains no-override guardrail', () => {
    expect(SCREENER_AUDITOR_SYSTEM_PROMPT).toContain('DO NOT override the signal engine');
  });

  it('contains no-fabrication guardrail', () => {
    expect(SCREENER_AUDITOR_SYSTEM_PROMPT).toContain('DO NOT invent entry, stop loss, take profit, leverage, liquidation');
  });
...
describe('buildAuditUserMessage', () => {
  it('includes only safe fields, no secrets', () => {
    ...
    // Must NOT include raw entry/SL/TP (AI must not see price levels)
    expect(parsed.entry).toBeUndefined();
    expect(parsed.stopLoss).toBeUndefined();
    expect(parsed.takeProfits).toBeUndefined();
  });
});
